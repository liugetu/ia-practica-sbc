...
;;; ---------------------------------------------------------
;;; run_query_all.clp
;;; Script per carregar l'ontologia, les dades d'inicialització i el sistema expert
;;; i executar la consulta de recomanacions
;;; ---------------------------------------------------------
...
;;; Reinicia i executa el sistema expert
(reset)
(run)

;;; Funció per obtenir el valor numèric del nivell de recomanació (per ordenar)
...
;;; Funció per mostrar recomanacions d'un client concret
(deffunction mostrar-recomanacions-client (?nom-client)
   ...
   ;; Només processar si el slot té un nom d'instància vàlid (no nil)
   ...
   ;; Recollir totes les recomanacions per a aquest client
   ...
   (do-for-all-instances
      ...
      ;; Guardar les dades de recomanació com a llista: [nivell-numèric, oferta, nivell-text, instància]
      (bind ?rec-data (create$ ?nivell-numeric ?oferta ?nivell ?r))
      (bind ?recomanacions (create$ ?recomanacions ?rec-data))
   )
   
   ;; Ordenar recomanacions per nivell (més alt primer)
   (if ?trobat then
      ;; Ordenació de bombolla simple per a recomanacions
...
;;; Funció per mostrar totes les recomanacions
